...
const api = createApi({
  ...
  tagTypes: ["Posts", "User", "Product", "Comments", "Stats", "Board", "Post"],
  endpoints: (builder) => ({
    ...
    getLatestPosts: builder.query<Post[], {field: string, lastId?: number}>({
      query: ({field, lastId}) => `products/latest_posts/${field}/${lastId || 0}`,
      providesTags: ["Posts", "User"],
    }),
    ...
    getPostDetails: builder.query<Post, { postId: string; channel: string }>({
      query: ({ postId, channel }) => `products/post/${channel}/${postId}`,
      providesTags: ["Post"]    
    }),
    ...
    getPostComments: builder.query<
      ...
      query: ({ postId, lastId }) => `products/comments/${postId}/${lastId || 0}`,
      providesTags: ["Comments"]    
    }),
    getChatMessages: builder.query<void, {productId: number, lastId?: number}>({
        query: ({productId, lastId}) => `products/messages/${productId}/${lastId || 0}`
    }),
