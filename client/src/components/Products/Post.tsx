...
export default function PostView({
  ...
  return (
    <div onClick={() => router.replace(`products/${data.product.name}/${channel}/${data.ID}`)} className={`${!showDivider && "mb-6 w-auto"} cursor-pointer`}>
      <div className="bg-white p-5 rounded-3xl  min-w-[20vw] mb-4 items-center mx-12 mt-4 md:mt-0">
        ...
        <div>
          ...
          <div className={'text-sm mt-2 text-gray-400 flex items-center'}>
              <p className="">{moment(new Date(data.UpdatedAt)).fromNow()}</p>
            <p className={"mx-1"}>|</p>
