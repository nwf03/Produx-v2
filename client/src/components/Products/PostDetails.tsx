...
export default function PostDetails() {
   ...
   const [getPostDetails, {error, data, isLoading}] = useLazyGetPostDetailsQuery()
    useEffect(() => {
        getPostDetails({
    ...
    return (
        <div>
            ...
            <div className="bg-white p-5 rounded-3xl  min-w-[20vw] mb-4 items-center mx-12 mt-4 md:mt-0">
                <button onClick={() => router.replace(`/products/${data.product.name}`)}>
                ...
                <div className="grid-cols-3 flex align-bottom mt-4">
                    <div className="flex items-center">
                        {data.user.pfp && (
                            <Avatar squared bordered src={data.user.pfp} size="xl" />
                        )}
                        <span className="font-bold text-xl pl-3 lg:mr-1 ">{data.title}</span>
            ...
            {channel.substring(0, channel.length - 1)}
                ...
                <div>
                    ...
                    <div className={'text-sm mt-2 text-gray-400 flex items-center'}>
                        <p className="">{moment(new Date(data.CreatedAt)).calendar()}</p>
                        <p className={"mx-1"}>|</p>
            ...
            {data.comments && data.comments.map((m,idx) => {
                return (
                    <div key={idx}>
