...
import { Avatar, Button, FormElement, Input } from "@nextui-org/react";
import {
  ChangeEvent,
  useState,
  useEffect,
...
export default function ProductUsers({
  ...
  useEffect(() => {
    ...
    if (auth) {
      ...
      console.log("puserid", productUserID);
    }
  }, []);
            ...
            <div key={idx} className={"block"}>
              {userID == productUserID ? (
                <div>
                  ...
                  >
                    {user.pfp && (
                      <Avatar size={"lg"} squared bordered src={user.pfp} />
                    )}
                  ...
                  {showUserInfo.show && showUserInfo.userId == user.id && (
                    <form className={"p-4 rounded-box w-11/12 bg-white "}>
                      <Input
                      ...
                      <button
                        ...
                        onClick={(e) => handleRoleUpdate(e, user.id)}
                      >
                        Change
                      </button>
                    </form>
