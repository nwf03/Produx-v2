import { Post, Comment } from '../../../../state/interfaces'
import PostView from './Post'
import CommentCard from './Comments/CommentCard'
import { Modal, Avatar, Button, Link, Tooltip } from '@nextui-org/react'
...
export default function PostDetails({ post }: { post?: Post }) {
    ...
    const [getComments, commentsReqInfo] = useLazyGetPostCommentsQuery()
    const [comments, setComments] = useState<Comment[]>([])
    const observer = useRef()
    // const [lastId, setLastId] = useState(0);
    const lastId = commentsReqInfo.data ? commentsReqInfo.data.lastId : 0
    const hasMore = commentsReqInfo.data ? commentsReqInfo.data.hasMore : true
    const pageEndRef = useCallback(
        (node) => {
            if (commentsReqInfo.isLoading) return
        ...
        [commentsReqInfo.isLoading, hasMore]
    )

    useEffect(() => {
        if (commentsReqInfo.data && commentsReqInfo.data.comments) {
            setComments([...comments, ...commentsReqInfo.data.comments])
            // setLastId(commentsReqInfo.data.lastId);
        }
    }, [commentsReqInfo.data])
    ...
    const isImg = (fileUrl: string) => {
        const imgTypes = ['png', 'jpg']
        const arr = fileUrl.split('.')
        return imgTypes.includes(arr[arr.length - 1])
    }
    return (
        <div>
            {data && (
                <div>
                    <div className="bg-white p-5 rounded-3xl  min-w-[20vw] mb-4 items-center mx-4 mt-4 md:mt-0">
                        ...
                        <div className="grid-cols-3 flex align-bottom mt-4">
                            ...
                            <div
                                className={'flex items-center my-auto ml-auto'}
                            >
                        ...
                        <div>
                            ...
                            {data.images && (
                                ...
                                >
                                    {data.images.map((img: string, idx) => {
                                        return (
                                            <div key={idx} className="mt-2">
                                                ...
                                                >
                                                    {isImg(img) ? (
                                                        <img
                                                            src={img}
                                                            className="rounded-box object-cover aspect-auto max-h-[400px]"
                                                        />
                                                    ) : (
                                                        <video controls>
                                                            <source
                            ...
                            {/* {showImgModal && <ImageModal show={showImgModal} setShow={setShowImgModal} src={selectedImg} img={isImg(selectedImg)}/>} */}
                            {/**/}
