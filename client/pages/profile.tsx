...
export default function Profile() {
  ...
  const [updateUser, updateUserReqInfo] = useUpdateUserMutation();
  ...
  const handleNewPfp = (e: ChangeEvent<HTMLInputElement>) => {
    if (fileRef.current.files) {
      ...
      setPfp(fileRef.current.files[0]);
  ...
  const [deleteProduct, { isLoading, error, data }] =
    useDeleteProductMutation();
  ...
  return (
    <div>
        ...
        {save && (
          <div className={"ml-2"}>
            <button
              ...
              className="btn btn-primary capitalize mr-5"
            >
              Save
            </button>
            <button
              className={"btn bg-white text-black border-1 border-black capitalize hover:bg-gray-200"}
              onClick={handleReset}
            >
              Reset
            </button>
              ...
              {userInfo.pfp && (
                <div className="relative hover:cursor-pointer">
                  ...
                  <div
                    ...
                    className="opacity-0 hover:opacity-100 w-[80vw] h-full md:w-full rounded-3xl flex absolute top-0  z-10"
                  >
                    <div className="  bg-black w-[80vw] h-full md:w-full rounded-3xl  opacity-50 absolute top-0 " />
                    <div
                      className={"form-control m-auto z-20 "}
                    >
              ...
              return (
                <div key={p.ID} className={"flex items-center"}>
                  ...
                  <Button
                    ...
                    onClick={() => handleDelete(p.name, p.ID)}
                  >
                    Delete
                  </Button>
                  {data && (
      ...
      <br />
      <ConfirmDelete
        productName={DelProduct.name}
        productID={DelProduct.id}
        isShown={showDelModal}
        setIsShown={setShowDelModal}
      />
    </div>
  );
}
